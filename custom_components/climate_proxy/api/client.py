...
class ClimateProxyApiClientError(Exception):
    """Base exception to indicate a general API error."""


class ClimateProxyApiClientCommunicationError(
    ClimateProxyApiClientError,
):
    """Exception to indicate a communication error with the API."""


class ClimateProxyApiClientAuthenticationError(
    ClimateProxyApiClientError,
):
...
def _verify_response_or_raise(response: aiohttp.ClientResponse) -> None:
    ...
    Raises:
        ClimateProxyApiClientAuthenticationError: For 401/403 errors.
        aiohttp.ClientResponseError: For other HTTP errors.

    """
    if response.status in (401, 403):
        msg = "Invalid credentials"
        raise ClimateProxyApiClientAuthenticationError(
            msg,
        )
    response.raise_for_status()


class ClimateProxyApiClient:
    """
    ...
    async def async_get_data(self) -> Any:
        ...
        Raises:
            ClimateProxyApiClientAuthenticationError: If authentication fails.
            ClimateProxyApiClientCommunicationError: If communication fails.
            ClimateProxyApiClientError: For other API errors.
    ...
    async def _api_wrapper(
        ...
        try:
            ...
            raise ClimateProxyApiClientCommunicationError(
                msg,
            ) from exception
            ...
            raise ClimateProxyApiClientError(
                msg,
            ) from exception
