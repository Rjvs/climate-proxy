...
- **Repository:** Rjvs/climate-proxy

**Key directories:**

- `custom_components/climate_proxy/` - Main integration code
...
- Integration logs: `custom_components.climate_proxy: debug` in `config/configuration.yaml`
...
- Use the domain `climate_proxy` for all DOMAIN references
- Prefix all integration-specific classes with `ClimateProxy`
- Use "Climate Device Proxy" as the display title
...
- Inherit from platform base + `ClimateProxyEntity`
...
- `tests/` mirrors `custom_components/climate_proxy/` structure
