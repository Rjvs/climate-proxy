...
This document describes all configuration options and settings available in the Climate Device Proxy custom integration.
...
```yaml
service: climate_proxy.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: climate_proxy.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
...
```yaml
blueprint:
  name: Climate Device Proxy Alert
...
- [GitHub Issues](https://github.com/Rjvs/climate-proxy/issues) - Report problems
